...
It should be stated that Chick-fil-A is a customer service oriented brand. They desire to potentially use the methods described in analysis as part of a larger project to discover how emotion relates to restaurant reviews in on-line comments and customer perception of service failure.
...
Plutchik's Wheel of Emotion, see Figure \ref{plutchikemotionwheel}, has eight basic emotions including: joy, trust, fear, surprise, sadness,disgust, anger, and anticipation. Each of the eight emotions has three levels of activation represented by color boldness. The emotion space is represented so that combinations of basic emotions derive secondary emotions. For example: joy+trust=love, anger+anticipation=aggression \cite{plutchik2001nature}.
...
Figure \ref{valepoch} shows the change in validation accuracy by epoch for a few different methods. We used both mono-directional and bi-directional neural nets with up to three hidden layers. Figure \ref{valepoch} shows the results for each of the methods. As we can see each method starts at approximately the same level of validation accuracy. The bidirectional method with two hidden layers appears to have the most consistent accuracy over the epochs, it levels off after five epochs without much fluctuation in validation accuracy. 
...
%\end{figure}
 ...
 \begin{figure}[htb]
	\centering
	\resizebox{4in}{!}{\input{PrimaryEmotionDataResults.tex}}
...
\begin{figure}[htb]
	\centering
	\resizebox{4in}{!}{\input{LabeledTwitterDataResults.tex}}
...
%	\centering
%	\input{meg.tex}}
%	\caption{Positive Sentiment Distributions for a Few Restaurants}\label{lbtwitresult}
%\end{figure}
